...
async def main(_, msg):
    ...
    try:
        ...
        for post in posts:
            if post.shortcode == post_id:
                L.download_post(post, target= f"-{post_id}")
                break
        path = f"-{post_id}"
...
**Note** : To get a profile picture of an account, use "`/profile_pic instagram-username`". The link won't work.
"""
